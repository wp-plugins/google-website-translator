...
class PrisnaGWTPermalinksField extends PrisnaGWTField {
	
	public $resource_formatted;
	public $permalink_structure_empty_message;
	public $permalinks_translations_saved_message;
	...
	protected function _initialize_resource_field($_properties) {
		...
		$_resource_field['site_url'] = PrisnaGWTCommon::getSiteHost();
	...
	protected function _get_files_scopes() {
		...
		$file = $this->folder . '/permalinks.xml';
		...
		$from = PrisnaGWTConfig::getSettingValue('from');
		$permalinks = $xpath->query("/permalinks/permalink/translation[@from='$from']/ancestor::permalink");
			
		if (!is_object($permalinks))
			return $result;
			
		foreach ($permalinks as $permalink) {
			$scope = $permalink->getAttribute('scope');
	...
	public function output($_template='permalinks.tpl', $_html_encode=false) {

		$permalink_structure = get_option('permalink_structure');

		$this->resource_formatted = $this->_resource_field->output('permalinks_select.tpl', $_html_encode);
